...
flowchart TD
    Start["🏁 Registration Created"] --> AutoCreate["🔄 Auto-Create Products<br/>(in this application)"]
    AutoCreate --> ManualCreate["📝 Manual Product Creation<br/>POST /v1/products"]
    ManualCreate --> Validate["✅ Validate Product Data"]
    Validate --> Create["📤 Create Product"]
    Create --> Success{"Creation<br/>Successful?"}
    Success -->|Yes| Ready["✅ Product Ready<br/>for Tax Calculations"]
    ...
    Ready --> UseInTxn["🛒 Use in Transactions<br/>Reference by external_product_id"]
