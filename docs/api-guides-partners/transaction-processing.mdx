...
Transaction processing is the core operational phase of your STaaS platform. This phase handles real-time tax calculation during checkout and records completed transactions for compliance tracking. All endpoints can be accessed via HTTP REST or the optional Kintsugi SDK.

## Transaction Processing Flow

<Expandable title="💳 Complete Transaction Flow" icon="diagram">
```mermaid
...
flowchart TD
    Start["🛒 Customer Checkout"] --> Prepare["📋 Prepare Transaction Data<br/>products, amounts, addresses"]
    Prepare --> TaxEst["Calculate Tax Estimate<br/>POST /v1/tax/estimate"]
    TaxEst --> Display["💰 Display Tax to Customer"]
    Display --> Payment["💳 Process Payment"]
    Payment --> Success{"Payment<br/>Successful?"}
    Success -->|No| Cancel["❌ Cancel Order<br/>Don't Create Transaction"]
    Success -->|Yes| CreateTxn["Create Transaction<br/>POST /v1/transactions"]
    CreateTxn --> Store["💾 Store Transaction ID"]
    Store --> Compliance["📋 Available for Compliance<br/>Nexus Tracking & Filings"]
    
    Compliance --> GetTxn["Get Transactions<br/>SDK: transactions.get()"]
    GetTxn --> Dashboard["📊 Display in Dashboard"]
