...
<Note>
**Which key endpoint to use.** `POST /v1/organizations/api_keys` (this step) mints a key tied to the newly created client organization during programmatic onboarding. To manage partner-owned keys (portfolio-wide or scoped to a client), use the Client API key endpoints instead — [`POST /v2/partner/api_keys`](/reference/partners/partner-management/api-keys/create-client-api-key), [`GET`](/reference/partners/partner-management/api-keys/list-client-api-keys), [`DELETE`](/reference/partners/partner-management/api-keys/delete-client-api-key) — or the Partner Experience UI under {/* TODO: confirm exact PX label */} **Settings → API Keys**. See [Authentication and API keys](/docs/api-guides-partners/overview#authentication-and-api-keys).
</Note>
