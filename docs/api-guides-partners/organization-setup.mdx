...
Organization setup is the foundation of every client relationship in your STaaS platform. This phase establishes the client's organization in Kintsugi, generates secure API credentials, and configures all business details required for tax compliance. These endpoints can be accessed via HTTP REST.
...
<Expandable title="📋 Complete Onboarding Flow" icon="diagram">
```mermaid
...
flowchart TD
    Start["🏁 Client Signs Up"] --> CreateOrg["1️⃣ Create Organization<br/>POST /v1/organizations"]
    CreateOrg --> StoreOrgID["💾 Store organization_id"]
    StoreOrgID --> CreateKey["2️⃣ Create API Key<br/>POST /v1/organizations/api_keys"]
    CreateKey --> StoreKey["💾 Store api_key securely"]
    StoreKey --> BusinessDetails["3️⃣ Set Business Details<br/>PUT /v1/organization_details/business_details/"]
    BusinessDetails --> BusinessAddress["4️⃣ Set Business Address<br/>PUT /v1/organization_details/business_address/"]
    BusinessAddress --> BusinessInfo["5️⃣ Set Business Information<br/>PUT /v1/organization_details/business_information/"]
    BusinessInfo --> BusinessContact["6️⃣ Set Business Contact<br/>PUT /v1/organization_details/business_contact/"]
    BusinessContact --> BusinessOwners["7️⃣ Set Business Owners<br/>PUT /v1/organization_details/business_owners/"]
    BusinessOwners --> BankDetails["8️⃣ Set Bank Details<br/>PUT /v1/bank_details/"]
    BankDetails --> Complete["✅ Onboarding Complete"]
...
<Note>
**Which key endpoint to use.** `POST /v1/organizations/api_keys` (this step) mints a key tied to the newly created client organization during programmatic onboarding. To manage partner-owned keys (portfolio-wide or scoped to a client), use the Partner-Organization API key endpoints instead — [`POST /v2/partner/api_keys`](/reference/partners/partner-management/api-keys/create-partner-organization-api-key), [`GET`](/reference/partners/partner-management/api-keys/list-partner-organization-api-keys), [`DELETE`](/reference/partners/partner-management/api-keys/delete-partner-organization-api-key) — or the Partner Experience UI under {/* TODO: confirm exact PX label */} **Settings → API Keys**. See [Authentication and API keys](/docs/api-guides-partners/overview#authentication-and-api-keys).
</Note>
