...
<Steps>
  ...
  <Step>
  Open the {/* TODO: confirm exact PX label */} **Settings** section.
  </Step>
  <Step>
  Select **API Keys** {/* TODO: confirm exact PX submenu wording */} to search, manage, and generate API keys for your organization.
  {/* TODO: replace with PX Settings → API Keys screenshot */}
  </Step>
  <Step>
  Create a new key, choose an expiration period, then copy the key immediately and store it securely — you will not be able to view it again.
  </Step>
</Steps>

<Note>
**Building white-label / Sales Tax as a Service?** For programmatic onboarding, partners create org-scoped keys for their clients via the API with `POST /v1/organizations/api_keys`. See the [Partner Organization Setup guide](/docs/api-guides-partners/organization-setup). This programmatic flow is unchanged.
</Note>
