...
```mermaid
graph TB
    Developer[Developer<br/>Building Integration] -->|Asks Question| AIClient[AI Assistant<br/>Cursor/Claude Desktop]
    AIClient -->|Queries via MCP| MCPServer[Kintsugi MCP Server]
    MCPServer -->|Reads OpenAPI Spec| OpenAPI[Kintsugi openapi.json<br/>API Documentation]
    MCPServer -->|Provides Context| AIClient
    AIClient -->|Generates Code| Code[Working Code<br/>Calls Kintsugi API]
    Code -->|HTTP Requests| KintsugiAPI[Kintsugi API<br/>api.trykintsugi.com]
    KintsugiAPI -->|API Responses| Code
    Code -->|Integration Complete| Developer
    
    style Developer fill:#e1f5ff
    style AIClient fill:#fff4e1
    style MCPServer fill:#e8f5e9
    style OpenAPI fill:#f3e5f5
    style Code fill:#ffe0e0
    style KintsugiAPI fill:#e0e0ff
```
