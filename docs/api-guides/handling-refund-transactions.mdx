...
flowchart TD
    Refund["💰 Refund Requested"] --> Find["🔍 Find Original Transaction<br/>GET /v1/transactions/external/{external_id}"]
    Find --> Check{"Transaction<br/>COMMITTED?"}
    Check -->|No| Update["🔄 Update to COMMITTED<br/>or Cancel"]
    Check -->|Yes| Prepare["📋 Prepare Credit Note Data<br/>match products, calculate amounts"]
    Update --> Prepare
    Prepare --> Validate["✅ Validate Amounts<br/>don't exceed original"]
    Validate --> Create["📤 POST /v1/transactions/{id}/credit_notes<br/>Create Credit Note"]
    Create --> Success{"Creation<br/>Successful?"}
    Success -->|Yes| Updated["✅ Original Transaction<br/>Refund Status Updated"]
    Success -->|No| Error["❌ Handle Error"]
    Error --> Retry{"Retry<br/>Appropriate?"}
    Retry -->|Yes| Create
    Retry -->|No| Fix["🔧 Fix Data Issues"]
    Fix --> Prepare
    Updated --> Complete["✅ Refund Recorded"]
...
<Expandable title="📊 Refund Status Tracking" icon="chart">
```mermaid
%%{init: {'theme':'base', 'themeVariables': {'primaryColor':'#FFFFFF','primaryTextColor':'#213331','primaryBorderColor':'#213331','lineColor':'#213331','secondaryColor':'#FFFFFF','tertiaryColor':'#213331','edgeLabelBackground':'#FFFFFF','edgeLabelColor':'#213331','edgeStrokeWidth':'2px','edgeStroke':'#213331','arrowheadColor':'#213331','clusterBkg':'transparent','clusterBorder':'#213331','defaultLinkColor':'#213331'}}}%%
flowchart TD
    Original["📝 Original Transaction<br/>$100.00"] --> Refund1["💰 Credit Note 1<br/>$30.00"]
    Refund1 --> Status1["🔄 Status: PARTIALLY_REFUNDED<br/>Remaining: $70.00"]
    Status1 --> Refund2["💰 Credit Note 2<br/>$70.00"]
    Refund2 --> Status2["🔄 Status: FULLY_REFUNDED<br/>Remaining: $0.00"]
...
- [Get Transaction by ID](/reference/api/transactions/get-transaction-by-id)
