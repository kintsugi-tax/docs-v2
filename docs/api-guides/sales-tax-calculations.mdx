...
The tax estimate endpoint (`POST /v1/tax/estimate`) calculates sales tax for transactions before payment is collected. It is the core of the **Level 2 (L2)** tax engine—enabled after transaction sync (L1) is in place. Tax calculation determines rates based on your nexus status, product taxability, customer exemptions, and shipping addresses. This guide covers when to call the estimate endpoint, how to structure requests, and how to use the tax amounts in your application.
...
<Note>
**Tax estimates don't sync transactions**: The `/v1/tax/estimate` endpoint calculates tax but doesn't create transaction records. For full compliance (L2), sync transactions separately using `/v1/transactions` after payment is confirmed. Kintsugi uses your synced transaction data to determine nexus—essential for accurate tax calculation even when Kintsugi is not handling filing and remittance.
</Note>
...
<Expandable title="💰 Tax Calculation Flow" icon="calculator">
```mermaid
...
flowchart TD
    Cart["🛒 Cart with Items"] --> Address["📍 Customer Enters Address"]
    Address --> Validate["✅ Validate Address<br/>POST /v1/address_validation/search"]
    Validate --> Prepare["📋 Prepare Estimate Request<br/>products, amounts, addresses"]
    Prepare --> Estimate["📊 POST /v1/tax/estimate<br/>Calculate Tax"]
    Estimate --> Response["📥 Tax Estimate Response<br/>tax amounts, jurisdictions"]
    Response --> Display["💰 Display Total<br/>with Tax"]
    Display --> Payment["💳 Process Payment"]
    Payment --> Success{"Payment<br/>Successful?"}
    Success -->|Yes| Sync["📝 Sync Transaction<br/>with Tax Amounts"]
    Success -->|No| Cart
