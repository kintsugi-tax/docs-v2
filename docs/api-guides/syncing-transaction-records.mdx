...
Transaction sync creates permanent records of completed sales in Kintsugi. These records power nexus tracking, compliance reporting, and tax filing preparation. This guide covers when to sync transactions, how to structure transaction data, and best practices for bulk imports and real-time syncing.
...
<Note>
**Transaction sync is separate from tax calculation**: The `/v1/transactions` endpoint creates records but doesn't calculate tax. If you need tax amounts, calculate them using `/v1/tax/estimate` before syncing, or include tax amounts in your transaction data.
</Note>
...
<Expandable title="📝 Transaction Sync Flow" icon="receipt">
```mermaid
...
flowchart TD
    Order["📦 Order Created"] --> Payment["💳 Payment Processing"]
    Payment --> Success{"Payment<br/>Successful?"}
    Success -->|No| Cancel["❌ Don't Sync<br/>Order Cancelled"]
    Success -->|Yes| Prepare["📋 Prepare Transaction Data<br/>products, amounts, addresses"]
    Prepare --> Validate["✅ Validate Data<br/>products exist, addresses valid"]
    Validate --> Create["📤 POST /v1/transactions<br/>status: COMMITTED"]
    Create --> Confirm["✅ Transaction Synced<br/>Available for Nexus Tracking"]
...
%%{init: {'theme':'base', 'themeVariables': {'primaryColor':'#FFFFFF','primaryTextColor':'#213331','primaryBorderColor':'#213331','lineColor':'#213331','secondaryColor':'#FFFFFF','tertiaryColor':'#213331','edgeLabelBackground':'#FFFFFF','edgeLabelColor':'#213331','edgeStrokeWidth':'2px','edgeStroke':'#213331','arrowheadColor':'#213331','clusterBkg':'transparent','clusterBorder':'#213331','defaultLinkColor':'#213331'}}}%%
flowchart TD
    Created["📝 Transaction Created<br/>status: PENDING"] --> Payment["💳 Payment Processing"]
    Payment --> Paid{"Payment<br/>Successful?"}
    Paid -->|Yes| Update["🔄 Update Status<br/>status: COMMITTED"]
    Paid -->|No| Cancel["❌ Update Status<br/>status: CANCELLED"]
    Update --> Processed["✅ Included in<br/>Nexus Calculations"]
    Update --> Filing["📋 Available for<br/>Filing Preparation"]
    Cancel --> Excluded["⏭️ Excluded from<br/>Compliance Tracking"]
